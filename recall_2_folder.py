...
def make_letter_text(pat, n):
    ...
    loader = FileSystemLoader(path_to_template)
    env = Environment(loader=loader)
    template_name = f"letter_{n}_template.txt"
    template = env.get_template(template_name)
    page = template.render(
        today_date=today_str,
        full_name=full_name,
...
def letter_compose(n):  # n is either "2" or "3"
    global recall_type
    recall_type = "letter"
    full_name = pat[0]
    title = full_name.split()[0]
...
scrape_button = ttk.Button(bottomframe, text="Get Email", command=scrape)
...
email_2_button = ttk.Button(
    bottomframe, text="Make Recall 2 Email", command=email_compose_2
)
email_2_button.grid(column=0, row=2, sticky=W)


letter_2_button = ttk.Button(
    bottomframe, text="Make Recall 2 letter", command=letter_compose_2
)
letter_2_button.grid(column=1, row=2, sticky=E)

email_3_button = ttk.Button(
    bottomframe, text="Make Recall 3 Email", command=email_compose_3
)
email_3_button.grid(column=0, row=3, sticky=W)


letter_3_button = ttk.Button(
    bottomframe, text="Make Recall 3 letter", command=letter_compose_3
)
letter_3_button.grid(column=1, row=3, sticky=E)


button_6 = ttk.Button(bottomframe, text="Finish & new", command=finish_recall)
button_6.grid(column=0, row=4, sticky=W)
